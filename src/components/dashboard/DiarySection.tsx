...
export default function DiarySection({ diaryEntries, onNewEntry }: DiarySectionProps) {
  ...
  const selectedEntry = diaryEntries.find(entry => new Date(entry.created_at).toDateString() === selectedDate.toDateString());
  ...
  const handleSaveMemory = async (memory: { imageUrl: string; context: string; mood: string }) => {
    if (!user) return;
    const entryForDate = diaryEntries.find(entry => new Date(entry.created_at).toDateString() === selectedDate.toDateString());

    if (!entryForDate) {
      showNotification({ message: "Save the diary entry first!", description: "Memories can be added after an entry is saved.", type: "error" });
      return;
    }
    const { error } = await supabase.from('memories').insert({ user_id: user.id, diary_entry_id: entryForDate.id, ...memory });
        ...
        <div className="bg-white border border-zinc-200 rounded-lg shadow-lg">
          ...
          <div className="p-6 pt-0">
            {isFuture ? <div className="p-4 text-center text-red-100 bg-red-600 rounded-md">You cannot write about the future... yet!</div> : (
              <RichDiaryEditor content={diaryHtml} isEditable={editMode} onChange={setDiaryHtml} onSaveMemory={handleSaveMemory} onDeleteImage={handleDeleteImage} showNotification={showNotification} />
            )}
            {!isFuture && (
              <div className="pt-4 flex gap-2">
                {editMode ? (
                  <>
                    <button onClick={handleSaveEntry} disabled={loading} className="inline-flex items-center justify-center h-10 px-4 py-2 text-sm font-medium text-white bg-zinc-900 rounded-md hover:bg-zinc-800 disabled:opacity-50">

...
import { 
    ...
    MapPin, Phone, Mail, Twitter, Facebook, Instagram, Play, Pause,TrendingUp, RotateCcw, LoaderCircle,Sparkles 
} from 'lucide-react';
import { PersonalizedRecommendations } from '../components/PersonalizedRecommendations.tsx'; // Make sure this path is correct

// --- Type Definitions ---
// For the Story Carousel, matching 'static_motivations'
interface Story {
  id: string; // Assuming 'id' is the primary key
  title: string;
  imagelink: string;
  category: string;
}

// For the Daily Motivation Player
    ...
    const fetchStories = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .limit(10); // Fetch up to 10 stories

        if (error) throw error;
        setStories(data || []);
      } catch (err: any) {
        setError("Could not fetch stories.");
        console.error("Error fetching static motivations:", err);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden group">
      <div className="relative h-80 overflow-hidden">
        ...
        <button
          ...
          className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 transition-all opacity-0 group-hover:opacity-100 z-10"
        >
          <ChevronLeft className="w-5 h-5" />
        </button>
        <button
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); goToNext(); }}
          className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 transition-all opacity-0 group-hover:opacity-100 z-10"
        >
...
function DailyMotivationPlayer() {
    ...
    useEffect(() => {
        const fetchDailyMotivation = async () => {
            ...
            try {
                ...
                else throw new Error("No motivational story found for today.");
            } catch (err: any) {
    ...
    if (isLoading) return (
        <div className="lg:col-span-2 bg-gray-200/50 backdrop-blur-sm p-8 rounded-2xl shadow-xl flex items-center justify-center h-[300px]">
            <LoaderCircle className="w-12 h-12 animate-spin text-indigo-500" />
        </div>
    );
    if (error || !motivation) return (
        <div className="lg:col-span-2 bg-red-100/50 p-8 rounded-2xl shadow-xl h-[300px] text-center flex flex-col justify-center">
            <h3 className="text-xl font-bold text-red-700">Something Went Wrong</h3>
            <p className="text-red-600 mt-2">{error || "Could not load today's motivation."}</p>
        </div>
    );
            ...
            <div className="relative z-10 p-6 md:p-8 flex flex-col justify-between h-full text-white">
                <div>
                    ...
                    <blockquote className="mt-2">
                        <p className="text-xl md:text-2xl font-bold italic">"{motivation.quote}"</p>
                        <cite className="block text-right mt-2 not-italic text-gray-300">- {motivation.name}</cite>
                    </blockquote>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-3 rounded-lg">
                    <audio 
                        ref={audioRef}
                        src={motivation.audiolink}
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={togglePlayPause} className="p-2 bg-white/20 rounded-full hover:bg-white/30">
                ...
                <div className="space-y-8">
                    <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-white/50">
                        ...
                        <div className="space-y-4">
                            <Link to="/diary" className="flex items-center gap-4 p-4 rounded-xl hover:bg-blue-50 border border-gray-100 hover:shadow-lg transition-all">
                                <div className="p-2 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-lg text-white"><BookOpen className="w-5 h-5" /></div>
                                <div><span className="font-medium text-gray-900">Write in Diary</span></div>
                            </Link>
                            <Link to="/community" className="flex items-center gap-4 p-4 rounded-xl hover:bg-green-50 border border-gray-100 hover:shadow-lg transition-all">
                                <div className="p-2 bg-gradient-to-r from-green-500 to-teal-500 rounded-lg text-white"><Users className="w-5 h-5" /></div>
                                <div><span className="font-medium text-gray-900">Community Support</span></div>
                            </Link>
                        </div>
                    </div>
                    {/* StoryCarousel now fetches its own data */}
                    <StoryCarousel />
                </div>
            </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 text-gray-800">
          <div className="md:col-span-2 lg:col-span-1">
            ...
            <form onSubmit={(e) => { e.preventDefault(); alert('Message sent!'); }} className="space-y-4">
              <input type="text" placeholder="Your Name" className="w-full px-4 py-2 bg-white/80 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all" required />
              <input type="email" placeholder="Your Email" className="w-full px-4 py-2 bg-white/80 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all" required />
              <textarea placeholder="Your Message" rows={3} className="w-full px-4 py-2 bg-white/80 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none transition-all" required></textarea>
              <button type="submit" className="w-full px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-semibold rounded-lg hover:from-indigo-600 hover:to-purple-600 transition-all shadow-lg hover:shadow-purple-500/30 transform hover:scale-105">Send Message</button>
            </form>
          </div>
          <div>
            <h3 className="text-xl font-bold mb-4 text-gray-900">Contact Us</h3>
            <ul className="space-y-3 text-gray-600">
              <li className="flex items-start gap-3"><MapPin className="w-5 h-5 mt-1 text-indigo-500 flex-shrink-0" /><span>123 Wellness Lane, Mindful City</span></li>
              <li className="flex items-center gap-3"><Phone className="w-5 h-5 text-indigo-500 flex-shrink-0" /><span>(123) 456-7890</span></li>
              <li className="flex items-center gap-3"><Mail className="w-5 h-5 text-indigo-500 flex-shrink-0" /><span>contact@ourwellnessapp.com</span></li>
            </ul>
          </div>
          <div className="md:col-span-2 lg:col-span-2">
            <div className="h-full min-h-[200px] bg-gray-200 rounded-2xl overflow-hidden shadow-lg border border-gray-200/50">
              <iframe
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3153.225829989444!2d144.9615393153163!3d-37.81729977975195!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6ad642af0f11fd81%3A0x5045675218ce7e0!2sMelbourne%20VIC%2C%20Australia!5e0!3m2!1sen!2sus!4v1620229873968!5m2!1sen!2sus"
                width="100%" height="100%" style={{ border: 0 }} allowFullScreen={false} loading="lazy" title="Office Location">
              </iframe>
            </div>
          </div>
        </div>
        <div className="mt-12 pt-8 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center">
          ...
          <div className="flex space-x-4 mt-4 sm:mt-0">
            <a href="#" className="text-gray-500 hover:text-indigo-600 transition-colors"><Twitter className="w-6 h-6" /></a>
            <a href="#" className="text-gray-500 hover:text-indigo-600 transition-colors"><Facebook className="w-6 h-6" /></a>
            <a href="#" className="text-gray-500 hover:text-indigo-600 transition-colors"><Instagram className="w-6 h-6" /></a>
          </div>
        </div>
      </div>
    </footer>
